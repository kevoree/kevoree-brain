 ...
 */
public class MovingPolynomial3 {
    public static void main(String[] arg) {
            ...
            for(int poly=4;poly<15;poly++){
                for(int size=poly+1; size<20; size++){
                    ...
                    for(int i=0;i<values.size();i++){
                        ...
                        err=Math.sqrt((h-y)*(h-y));
                        totalerr+=err;
                        counter++;
                    }

                    totalerr=totalerr/counter;
                    if(totalerr<besterr||besterr<0){
                        ...
                        System.out.println("best init "+ bestInit);
        ...
        try {
            ...
            for(int i=0;i<bestcoef.size();i++){
                double[] dd = bestcoef.get(i);
                for(double d: dd){
                   out2.print(d+" , ");
                }
                out2.println();
            ...
            out2.close();
            for(int i=0;i<values.size();i++){
                int group= i/(bestInit*bestdegr);
                if(group>=bestcoef.size())
                    break;
                double time=((double)(i-group*bestInit*bestdegr))/(bestdegr);
                double h=reconstruct(time,bestcoef.get(group));
                double y= values.get(i);
                double err= Math.sqrt((h-y)*(h-y));
