 ...
 */
public class CatAndMouse2 implements RLWorld {
    public int bx, by; //world dimension

    public int mx, my; //mouse position
    public int cx, cy; //cat position
    public int chx, chy; //cheese position
   // public int hx, hy;
    ...
    static final int NUM_OBJECTS=4, NUM_ACTIONS=8, WALL_TRIALS=100;
    ...
    public CatAndMouse2(int x, int y, int numWalls) {
        bx = x;
        by = y;
    ...
    public void initCheese(){
        ...
        for(int i=0;i<WALL_TRIALS;i++){
            int a= rand.nextInt(bx);
            int b=rand.nextInt(by);
            if(walls[a][b]==false){
                chx=a;
                chy=b;
                return;
    ...
    public CatAndMouse2(int x, int y, boolean[][] newwalls) {
        bx = x;
        by = y;
        walls = newwalls;
        initCheese();
